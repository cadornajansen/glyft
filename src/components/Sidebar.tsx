...
const PRESETS: Preset[] = [
  { name: 'Open Graph (OG) Image', width: 1200, height: 630, category: 'Web', icon: ShareIcon },
  { name: 'Hero Banner (Standard)', width: 1200, height: 480, category: 'Web', icon: Monitor },
  { name: 'Blog Cover', width: 1000, height: 500, category: 'Web', icon: BookOpen },
  { name: 'Product Card UI', width: 600, height: 600, category: 'Commerce', icon: Layout },
  { name: 'Social Post (Square)', width: 1080, height: 1080, category: 'Social', icon: Smartphone },
  { name: 'App Icon / Favicon', width: 512, height: 512, category: 'App', icon: ImageIcon },
];
...
export function Sidebar({ onLoadProject, onAddImage, onAddSVG }: SidebarProps) {
  const { 
    currentProjectId, 
    activeTab, 
    setActiveTab, 
  ...
  const saveRename = async (project: Project) => {
    if (editName.trim()) {
      const updated = { ...project, name: editName.trim(), updatedAt: Date.now() };
      ...
      <div className="flex border-b border-[#222] p-2 gap-1.5 bg-[#0a0a0a] items-center">
        <div className="relative flex-1">
          <button
            ...
            className={`relative z-10 w-full flex items-center justify-center gap-2 py-1.5 px-3 text-xs font-semibold rounded-md transition-colors ${
              activeTab === 'projects' ? 'text-white' : 'text-[#707070] hover:text-[#a1a1a1]'
            }`}
          >
            <Folder size={13} />
            <span>Projects</span>
        ...
        <div className="relative flex-1">
          <button
            ...
            className={`relative z-10 w-full flex items-center justify-center gap-2 py-1.5 px-3 text-xs font-semibold rounded-md transition-colors ${
              activeTab === 'assets' ? 'text-white' : 'text-[#707070] hover:text-[#a1a1a1]'
            }`}
          >
            <Sparkles size={13} />
            <span>Templates</span>
          ...
          {activeTab === 'projects' ? (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-widest font-bold text-[#555]">Local Storage</span>
                <button
                  id="btn-new-project"
              ...
              <div className="space-y-2">
                {projects.length === 0 ? (
                  <div className="text-center py-8 px-4 rounded border border-dashed border-[#222] bg-[#1a1a1a]/10">
                    ...
                    <p className="text-xs text-[#707070] font-medium">No assets created yet.</p>
                    <button
                      onClick={() => handleCreateNew('Untitled Graphic', 1200, 630)}
                    ...
                    >
                      {deletingId === proj.id ? (
                        <div className="flex flex-col gap-2 py-1" onClick={(e) => e.stopPropagation()}>
                          <span className="text-xs font-semibold text-red-400">Delete this project?</span>
                          <span className="text-[10px] text-[#707070] leading-relaxed">This action is irreversible and cannot be undone.</span>
                            ...
                            <button
                              ...
                              onClick={async () => {
                                ...
                                if (currentProjectId === proj.id) {
                                  ...
                                  useEditorStore.getState().setCurrentProject(null);
                                }
                                setDeletingId(null);
                                loadProjects();
                              }}
                        ...
                        <>
                          ...
                          <div className="flex justify-between items-center mt-3 text-[10px] text-[#707070]">
                            ...
                            <span>{new Date(proj.updatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                          </div>
            ...
            >
              ...
              <div className="space-y-3">
                <span className="text-[10px] uppercase tracking-widest font-bold text-[#555]">Starter Presets</span>
                <div className="grid grid-cols-1 gap-2">
                  {PRESETS.map((preset) => (
                    <button
                      key={preset.name}
                      id={`preset-${preset.name.replace(/\s+/g, '-').toLowerCase()}`}
                      onClick={() => handleCreateNew(preset.name, preset.width, preset.height)}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-[#707070] mt-0.5">{preset.width} × {preset.height} px • {preset.category}</div>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="space-y-3">
                <span className="text-[10px] uppercase tracking-widest font-bold text-[#555]">Import Assets</span>
                <div
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-[#a1a1a1]">Drag & Drop Image</span>
                  <span className="text-[10px] text-[#707070] mt-1">PNG, JPG, SVG, or WebP</span>
